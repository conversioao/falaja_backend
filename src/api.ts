...
app.post('/api/transcribe', express.raw({ type: '*/*', limit: '50mb' }), authenticateToken, async (req: any, res) => {
    try {
        ...
        // Calculate initial credit deduction (Fair rounding: ceil(duration_mins * multiplier))
        const initialUsedMinutesInc = Math.ceil(((initialDurationSec || 0) / 60) * multiplier);
        ...
        if (user && (user.credits || 0) <= 2 && initialUsedMinutesInc === 0) {
...
app.post('/api/webhooks/transcription-complete', async (req, res) => {
    ...
    try {
        ...
        // Calculate credits based on the NEW duration from n8n (Fair rounding)
        const newUsedMinutesInc = Math.ceil(((finalDurationSec || 0) / 60) * multiplier);

        // Calculate credits based on the PREVIOUS duration (already deducted if using placeholders) (Fair rounding)
        const prevUsedMinutes = Math.ceil(((prevDurationSec || 0) / 60) * multiplier);
...
app.post('/api/recordings', authenticateToken, async (req: any, res) => {
    ...
    try {
        ...
        const usedMinutesInc = Math.ceil(((durationSec || 0) / 60) * multiplier);
