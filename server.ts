...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        const result = await query('INSERT INTO users (name, whatsapp, password_hash, verification_code, is_verified, credits, used_minutes, app_mode, role, plan, email) VALUES ($1, $2, $3, NULL, true, $4, 0, $5, $6, $7, $8) RETURNING id, name, whatsapp, plan, credits, used_minutes as "usedMinutes", avatar_url as "avatarUrl", app_mode as "appMode", role', 
            [name, whatsapp, passwordHash, welcomeBonus, 'professional', 'user', 'Gratuito', `${whatsapp}@falaja.ao`]);
        const user = result.rows[0];
        const token = generateToken({ id: user.id });

        // Cleanup user object for response
        user.usedMinutes = user.used_minutes || user.usedMinutes || 0;
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        // Bypass verification check as all users are now verified by default
        /* 
        if (!user.is_verified) {
