...
app.patch('/api/recordings/:id/read', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    try {
...
app.post('/api/transcribe', express.raw({ type: '*/*', limit: '50mb' }), authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const initialDuration = (req.headers['x-duration'] || '00:00') as string;
        const initialDurationSec = parseInt((req.headers['x-duration-sec'] || '0') as string) || 0;
...
// --- WEBHOOKS ---
app.post('/api/webhooks/transcription-complete', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (finalTitle && finalTitle.length > 80) finalTitle = finalTitle.substring(0, 77) + '...';
...
app.post('/api/recordings', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
        ...
        const user = userRes.rows[0];
        if (user && user.credits <= 0) return res.status(402).json({ error: 'Saldo insuficiente.' });
...
app.patch('/api/recordings/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
...
app.put('/api/admin/settings', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
    try {
