...
app.post('/api/transcribe', express.raw({ type: '*/*', limit: '50mb' }), authenticateToken, async (req, res) => {
    try {
        ...
        const settingsMap = {};
        settingRes.rows.forEach((row) => settingsMap[row.setting_key] = row.setting_value);
        ...
        const durationSec = parseInt(req.headers['x-duration-sec'] || '0');
        ...
        fetch(webhookUrl, {
            method: 'POST',
            headers: { 'Content-Type': req.headers['content-type'] || 'audio/wav', 'X-User-Id': req.userId.toString(), 'X-Mode': mode, 'X-Recording-Id': recordingId.toString(), 'X-Callback-Url': callbackUrl, 'X-User-Name': encodeURIComponent(user.name), 'X-User-Whatsapp': user.whatsapp },
            body: req.body
...
app.post('/api/webhooks/transcription-complete', async (req, res) => {
    ...
    try {
        ...
        await query('BEGIN');
        await query('UPDATE recordings SET transcription = $1, summary = $2, action_items = $3, status = \'completed\', duration_sec = $4, duration = $5, title = COALESCE($6, title), is_read = false WHERE id = $7', [transcription, summary, actionItems, finalDurationSec, formatDuration(finalDurationSec), n8nTitle, recordingId]);
...
app.post('/api/recordings', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const result = await query('INSERT INTO recordings (user_id, title, duration, duration_sec, type, transcription, summary, action_items, is_read) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, false) RETURNING *', [req.userId, title, duration, durationSec, type, transcription, summary, actionItems]);
        await query('COMMIT');
...
app.patch('/api/recordings/:id', authenticateToken, async (req, res) => {
    ...
    try {
        const result = await query('UPDATE recordings SET title = COALESCE($1, title), transcription = COALESCE($2, transcription), summary = COALESCE($3, summary), action_items = COALESCE($4, action_items), is_read = COALESCE($5, is_read) WHERE id = $6 AND user_id = $7 RETURNING id, title, transcription, summary, action_items as "actionItems", is_read as "isRead"', [title, transcription, summary, actionItems, isRead, req.params.id, req.userId]);
...
app.patch('/api/user/mode', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/payments/:id/trigger-webhook', authenticateToken, requireAdmin, async (req, res) => {
    try {
