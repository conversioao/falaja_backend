...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        await query('INSERT INTO users (name, whatsapp, password_hash, verification_code, is_verified, credits, used_minutes, app_mode, role, plan, email) VALUES ($1, $2, $3, $4, false, $5, 0, $6, $7, $8, $9)', [name, whatsapp, passHash, code, bonus, 'professional', 'user', 'Gratuito', `${whatsapp}@falaja.ao`]);
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (!user.is_verified)
            return res.json({ needsVerification: true, message: 'Verifique WhatsApp' });
...
app.post('/api/transcribe', express.raw({ type: '*/*', limit: '50mb' }), authenticateToken, async (req, res) => {
    try {
        ...
        const settingsMap = {};
        settings.rows.forEach((r) => settingsMap[r.setting_key] = r.setting_value);
        ...
        fetch(webhookUrl, {
            method: 'POST',
            headers: { 'Content-Type': req.headers['content-type'], 'X-Recording-Id': recording.rows[0].id.toString(), 'X-Callback-Url': callbackUrl },
            body: req.body
        });
